...
const MedicalForm = () => {
  ...
  useEffect(() => {
    ...
    const delay = setTimeout(fetchSuggestions, 300); // Debounce API call
  ...
  const generatePDF = async () => {
    try {
      await savePrescription(); // Ensure data is saved before generating PDF
      ...
      /*<div className="medicine-list">
        ...
        <ul>
          {medications.map((med, index) => (
            <li key={index}>
              {med.medicine} - {med.dosage} - {med.days} days, {med.remark}
              <button onClick={() => setMedications(medications.filter((_, i) => i !== index))}>❌</button>
            </li>
          ))}
        </ul>
      </div>
